...
The batch scheduler makes scheduling decisions for each job based on its demand while trying to 
minimize overall job response time and maximize system throughput. 
However, jobs require for more than one resource could result in possible deadlock and starvation.
...
Upon the job completion, useful output data are staged out to the burst buffer in order to release compute node early.
