...
which could shift the computation that follows the I/O bursts to an earlier moment,
...
a particular job could be loaded into burst buffer prior to the execution;
Economical solid-state disks as a tier of burst buffer could also be used as
out-of-core complement to insufficient main memory\cite{Romanus:CORR:15},
working place for data analysis (reductions, feature extraction compression etc.)
and visualization\cite{TrinitySystem}.
...
traditional batch job queueing subsystem.
The bridge builds upon a 3-phase job model that is motivated by critical use cases of burst buffer, i.e., application checkpoint restart and data pre-fetch/cache.
