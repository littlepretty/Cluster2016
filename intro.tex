...
the preliminary results demonstrate that the 3-phase design of Cerberus 
...
\begin{enumerate}
        \item    We propose a 3-phrase job model (i.e., stage-in, running, and stage-out) 
        to describe user jobs and further to facilitate job scheduling in a fine granularity.
        
        \item    We present Cerberus, 
        a 3-phase burst buffer aware scheduler for HPC systems equipped with burst buffer. 
...
Section~\ref{Sec:Model} elaborates the 3-phase job model.
Section~\ref{Sec:Scheduler} and Section~\ref{Sec:Opt} describe the design of Cerberus and the optimization strategies.
