...
takes job size (required number of nodes) and expected runtime for scheduling decision making. 
...
Once the job gets the required number of compute nodes, it can start to run. There might be frequent 
data exchange between burst buffer and compute nodes during the running phase. 
...
Cerberus can schedule the jobs in each queue separately, 
rather than being held in a particular queue, waiting for certain resources.
...
the scheduling objective is modeled as a knapsack problem by defining the value of the $j_i$ as
\begin{equation}
...
\NOTE{Too much algorithm details in this section, may not be a good thing}
